...
------------------------------------------------------------------------------------
--
--  Lua - Script to compute a sedimenting 2d-particle with circular shape. It
...
--
--  J. Blasco, M.C. Calzada, M. Marín: 'A fictitious domain, parallel 
--   'numerical method for rigid particulate flows'
--  J. Comput. Phys., 228(20), 2009, pp. 7596–7613.
--
--  and
--
...
--  in: P.G. Ciarlet, J.L. Lions (Eds.), Handbook of Numerical Analysis IX,
--  North-Holland, Amsterdam, 2003.
--
--  Numerical results for the method implemented here are published in:
--
--  S. Höllbacher, G.Wittum: 'Rotational test spaces for a fully-implicit
--   'FVM and FEM for the DNS of fluid-particle interaction'
...
numTimeSteps = util.GetParamNumber("-numTimeSteps", 200)  -- compute until T = 2.0
...
-- = true: uses the usual ansatz- and test-spaces on a cut element, i.e. non-conforming. As in the
--         original ficticious domain method

print_cutElemData = true
-- = true: prints computed coordinates of the corners of cut elements and their local indices to
--         the file './CutElementData.txt'
...
-- add one particles with circular shape to the discretisation
...
-- In a first step we have to prepare the particle class. It sets the marker for the
...
vtkWriter:print("Sedimenting_Disc_GL"..numRefs, u, step, time, false)
...
for step = 1, numTimeSteps do
    ...
    filename_output_vel = "./Sedimenting_Disc_GL"..numRefs..".txt"
