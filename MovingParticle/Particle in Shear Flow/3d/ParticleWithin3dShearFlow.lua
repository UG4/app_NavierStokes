...
-----------------------------------------------------------------------------------
--
--  Lua - Script to compute a 3d-particle with spherical shape within shear flow. It
--  applies the immersed interface method provided by the 'MovingInterface' class.
--  Benchmark computations and reference data are provided (only for the 2d case) by
--
...
dt           = util.GetParamNumber("-dt", 0.02)
numTimeSteps = util.GetParamNumber("-numTimeSteps", 15000)  -- compute until T = 300.0

InitUG(dim, AlgebraType("CPU", dim+1));

if 	dim == 3 then
    gridName = util.GetParam("-grid", "grids/shear3d_unstructured_scaled_4_4_6.ugx")
...
function ZeroSolution(x, y, z, t)
    return 0.0
end

function LinearShearInlet(x, y, z, t)
    return 0.0, 0.0, 0.5*shearVel*y
end
...
InletDisc:add("LinearShearInlet", "Left")
InletDisc:add("LinearShearInlet", "Right")
InletDisc:add("LinearShearInlet", "Back")
InletDisc:add("LinearShearInlet", "Front")
...
ShearWall:add(0.0, "u", "Top")
ShearWall:add(0.0, "v", "Top")
ShearWall:add(shearVel, "w", "Top")

ShearWall:add(0.0, "u", "Bottom, PressureBndCond")
ShearWall:add(0.0, "v", "Bottom, PressureBndCond")
ShearWall:add(-shearVel, "w", "Bottom, PressureBndCond")
...
vtkWriter:print("Particle_in_3d_shear_flow_GL"..numRefs, u, step, time, false)
...
for step = 1, numTimeSteps do
    ...
    filename_output_vel = "./Particle_in_3d_shear_flow_GL"..numRefs..".txt"
...
vtkWriter:select_nodal("u,v,w", "velocity")
vtkWriter:select_nodal("p", "pressure")
vtkWriter:print("Particle_in_3d_shear_flow_GL"..numRefs, u)
